...
import OperationLogManagementView from '@/views/OperationLogManagementView.vue'; // 导入操作日志页面组件。
...
import AppUiAutomationView from '@/features/app-ui-automation/views/AppUiAutomationView.vue'; // 导入 APPUI 自动化页面组件。
...
const routes: Array<RouteRecordRaw> = [ // 声明路由表数组，类型约束为 RouteRecordRaw。
  ...
  },
  {
    ...
    children: [
      ...
      {
        path: 'operation-logs', // 定义操作日志子路径。
        name: 'OperationLogManagement', // 定义操作日志路由名称。
        component: OperationLogManagementView, // 指定操作日志页面组件。
      },
      {
      ...
      {
        path: 'app-ui-automation', // 定义 APPUI 自动化子路径。
        name: 'AppUiAutomation', // 定义 APPUI 自动化路由名称。
        component: AppUiAutomationView, // 指定 APPUI 自动化页面组件。
      },
      {
