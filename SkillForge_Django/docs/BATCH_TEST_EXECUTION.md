...
```bash
# 进入项目目录
cd C:\Code\skillforge\SkillForge_Django

# 启动Worker (Windows使用solo模式)
uv run celery -A skillforge_django worker --loglevel=info --pool=solo

# Linux/Mac可以使用默认模式
uv run celery -A skillforge_django worker --loglevel=info
```
...
在 `skillforge_django/settings.py` 中:
...
**A: 这是Celery在Windows上的已知兼容性问题。我已经通过修改 `skillforge_django/celery.py` 文件自动修复了此问题。**
