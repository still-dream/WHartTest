...
```bash
# 构建 Docker 镜像
docker build -t skillforge-django .
```
...
# 使用 .env 文件运行
docker run -d -p 8000:8000 --env-file .env skillforge-django
```
...
uvicorn skillforge_django.asgi:application --host 0.0.0.0 --port 8000 --workers 4
```
