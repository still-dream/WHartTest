"""
skillforge_django 项目的 Django 配置。
...
from skillforge_django.data_variant import get_postgres_db_name, get_sqlite_db_path
...
ASGI_APPLICATION = "skillforge_django.asgi.application"
...
# 指定项目根路由模块。
ROOT_URLCONF = "skillforge_django.urls"

# 配置模板引擎与上下文处理器。
TEMPLATES = [
    {
        "BACKEND": "django.template.backends.django.DjangoTemplates",
        "DIRS": [BASE_DIR.parent / "SkillForge_Vue" / "dist"],
        "APP_DIRS": True,
        "OPTIONS": {
            "context_processors": [
...
# 指定 WSGI 入口（传统同步部署场景）。
WSGI_APPLICATION = "skillforge_django.wsgi.application"
...
REST_FRAMEWORK = {
    ...
    "DEFAULT_PERMISSION_CLASSES": [
        ...
        "skillforge_django.permissions.DjangoModelPermissions",  # 使用扩展后的模型权限控制。
    ],
    # 默认过滤后端列表。
    ...
    "DEFAULT_RENDERER_CLASSES": (
        "skillforge_django.renderers.UnifiedResponseRenderer",
...
LOGGING = {
    ...
    "handlers": {
        ...
        "app_file": {
            "level": "INFO",
            "class": "skillforge_django.safe_log_handler.SafeTimedRotatingFileHandler",
...
SKILL_STORE_DEFAULT_SOURCE = os.environ.get(
    ...
    "https://gitee.com/duanxiangchun/didactic-octo-spork/raw/main/SkillForge_Skills/",
)
